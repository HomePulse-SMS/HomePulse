...
const SelectCity = ({ route, navigation }) => {
        ...
        fetch('http://192.168.0.183:8080/admin/getAllSociety', {
            ...
            .then(fetchedData => {
                const snameList = fetchedData.data
                    .filter(item => item && item.location && item.location.state === state)
    ...
    const handleCitySelect = (subcity) => {
        navigation.navigate('SelectSubCity', { subcity });
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            <Text style={styles.title}>🏙️ Select Your City</Text>
            <FlatList
                data={cities}
